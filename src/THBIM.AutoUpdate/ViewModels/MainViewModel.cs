...
public class MainViewModel : ViewModelBase
{
    ...
    public ObservableCollection<ReleaseHistoryItem> ReleaseHistory { get; } = [];

    private bool _hasReleaseHistory;
    public bool HasReleaseHistory
    {
        get => _hasReleaseHistory;
        set => SetProperty(ref _hasReleaseHistory, value);
    }
    ...
    public async Task CheckForUpdatesAsync()
    {
        ...
        try
        {
            ...
            // Refresh release history list (shown on Addins tab below the
            // up-to-date message). Fire-and-forget so check flow is not blocked.
            _ = LoadReleaseHistoryAsync(addinConfig, manifestVersion);
    ...
    private async Task LoadReleaseHistoryAsync(AddinConfig addinConfig, string currentInstalled)
    {
        try
        {
            var releases = await _githubService.GetRecentReleasesAsync(addinConfig.Owner, addinConfig.Repo, count: 10);
            if (releases.Count == 0) return;

            // Fetch manifest.json for each release in parallel; fallback to GitHub release body lines.
            var items = new List<ReleaseHistoryItem>();
            foreach (var rel in releases)
            {
                var version = rel.TagName.TrimStart('v', 'V');
                List<string> lines;

                var manifest = await _githubService.GetManifestAsync(rel);
                if (manifest?.Changelog is { Count: > 0 })
                    lines = manifest.Changelog.ToList();
                else
                    lines = ExtractBulletLines(rel.Body);

                items.Add(new ReleaseHistoryItem
                {
                    Version = version,
                    PublishedAt = rel.PublishedAt,
                    ChangelogLines = lines,
                    IsCurrent = string.Equals(version, currentInstalled, StringComparison.OrdinalIgnoreCase)
                });
            }
            ...
            await Application.Current.Dispatcher.InvokeAsync(() =>
            {
                ...
                foreach (var it in items) ReleaseHistory.Add(it);
                HasReleaseHistory = ReleaseHistory.Count > 0;
            });
        }
        catch
        {
    ...
    private async Task UninstallAsync()
    {
        const string warning =
            "Uninstall THBIM\n\n" +
            "This will:\n\n" +
            "  • Remove ALL THBIM addins from every installed Revit version\n" +
            "  • Delete your locally stored login session, cached profile,\n" +
            "    privacy consent record, and activity logs\n" +
            "  • Delete the AutoUpdate app's own settings and registry entries\n" +
            "  • Self-delete the AutoUpdate executable\n\n" +
            "Server-side data (your account record on our license server) is\n" +
            "NOT removed by this action. To request server-side erasure under\n" +
            "GDPR Art. 17, please email the publisher with subject\n" +
            "\"Delete my account\" before continuing.\n\n" +
        ...
        // Step 2: Remove app (which also wipes %LOCALAPPDATA%\THBIM\Licensing
        // including session, log, consent stamp — see UninstallService.RemoveApp).
