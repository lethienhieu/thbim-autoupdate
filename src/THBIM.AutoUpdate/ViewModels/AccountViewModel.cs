...
public class AccountViewModel : ViewModelBase
{
    ...
    public AccountViewModel(LicenseService licenseService)
    {
        ...
        // GDPR — initial consent state from the persisted stamp file. The XAML
        // CheckBox in MainWindow binds two-way to HasConsented, and LoginCommand
        // is gated on it so the Sign In button visibly indicates consent is required.
        _hasConsented = _licenseService.HasUserConsented();

        LoginCommand = new RelayCommand(async () => await LoginAsync(),
            () => !IsBusy && !string.IsNullOrWhiteSpace(Email) && HasConsented);
        ...
        ClearAllDataCommand = new RelayCommand(DoClearAllData);
    ...
    public bool HasConsented
    {
        ...
        set
        {
            ...
            {
                // Persist immediately when the user ticks the box so the consent
                // stamp file is written even if they back out before signing in.
                if (value) _licenseService.RecordUserConsent();
            }
        }
    }

    private void DoClearAllData()
    {
        const string body =
            "This will PERMANENTLY delete all THBIM local data on this computer:\n\n" +
            "  • Login session and authentication token\n" +
            "  • Locally stored profile (email, name, tier)\n" +
            "  • Privacy consent record\n" +
            "  • Activity log\n\n" +
            "You will be signed out and must log in again to use the service.\n\n" +
            "NOTE: To also delete your account on the licensing server, send an\n" +
            "email request to the publisher (see PRIVACY.md). This button only\n" +
            "removes data from THIS machine.\n\n" +
            "Continue?";
        var confirm = System.Windows.MessageBox.Show(
            body,
            "THBIM AutoUpdate — Delete My Data",
            System.Windows.MessageBoxButton.YesNo,
            System.Windows.MessageBoxImage.Warning);
        if (confirm != System.Windows.MessageBoxResult.Yes) return;

        bool ok = _licenseService.ClearAllUserData();

        // Reset UI state so the user sees the empty profile immediately.
        FullName = "";
        Tier = "FREE";
        ExpiryDisplay = "-";
        LicenseKey = "";
        Email = "";
        Password = "";
        // GDPR — reset the consent flag too so the user must re-accept before
        // signing back in (and the Sign In button is disabled until they do).
        _hasConsented = false;
        OnPropertyChanged(nameof(HasConsented));
        StatusMessage = ok ? "All local data deleted." : "Some files could not be removed (close Revit and retry).";
        IsError = !ok;
        IsLoggedIn = false;
    ...
    {
        // Consent is gated by the XAML CheckBox bound to HasConsented (see
        // MainWindow.xaml login section + LoginCommand CanExecute predicate).
        // Persist again here in case the stamp file was wiped between consent
        // and login (e.g., user clicked Delete My Data then signed back in).
        if (HasConsented) _licenseService.RecordUserConsent();
