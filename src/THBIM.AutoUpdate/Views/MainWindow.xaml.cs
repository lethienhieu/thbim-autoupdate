...
public partial class MainWindow : Window
{
    ...
    private void PrivacyPolicy_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            // Prefer local PRIVACY.md shipped next to the executable
            var dir = System.IO.Path.GetDirectoryName(Environment.ProcessPath);
            if (!string.IsNullOrEmpty(dir))
            {
                var local = System.IO.Path.Combine(dir, "PRIVACY.md");
            ...
            Process.Start(new ProcessStartInfo("https://thbim.pages.dev/privacy") { UseShellExecute = true });
        }
        catch { }
    }
